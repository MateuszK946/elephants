...
data = pd.read_csv(r"File path", header=None) # wczytywanie danych z pliku
df = data.copy()
...
numbre_of_elephants = int(b[0]) # ilość sloni
weight_of_elephants = [int(x) for x in b[1].split()] # wagi słoni
current_positions = [int(x) for x in b[2].split()] # obecna pozycje słoni
expected_positions = [int(x) for x in b[3].split()] # pozycje finalne słoni
